...
const PlatformMigration = () => {
  return (
    <Article>
      ...
      />
      <TextBlock 
        text="WhatNow's backend was originally deployed on Railway, a platform I chose for its simplicity and generous free tier. The initial deployment worked beautifully: push to GitHub, automatic deployment, PostgreSQL database included, environment variables managed through their dashboard. Development moved quickly." 
        sectionTitle="Railway to Render" 
      />
      <TextBlock 
        text="Then Railway's trial period ended, and the free tier became insufficient for active development. I needed to migrate to Render, another platform with a different free tier structure, without losing functionality." 
        sectionTitle="Migration Necessity" 
      />
      <TextBlock 
        text="The first migration attempt failed immediately: scikit-learn wouldn't compile on Render's build servers. Railway had handled scikit-learn's compilation requirements without issue, but Render's more restrictive build environment couldn't compile the native dependencies. The error logs showed compilation failures for numpy's C extensions and scipy's Fortran dependencies. Upgrading Python from 3.11 to 3.13 helped but didn't solve the core problem: Render's free tier simply couldn't support heavy ML dependencies." 
        sectionTitle="The scikit-learn Problem" 
      />
      <TextBlock 
        text="The solution was to implement a lightweight SGD classifier using only numpy and scipy, both of which compiled successfully on Render. I created LightweightSGDClassifier with the exact same interface as sklearn's SGDClassifier: partial_fit for online learning, predict_proba for scoring, and coef_/intercept_ for weight access." 
        sectionTitle="Lightweight Custom Implementation" 
      />
      <CodeBlock
        language="python"
...
# Faster for small batches, no compilation issues`}
      ...
      />
      <TextBlock 
        text="Migrating the database required careful attention to connection strings and SSL configuration. PostgreSQL on Railway used external connections without SSL requirements; Render's PostgreSQL requires SSL with specific parameters. The connection string needed sslmode=require appended to work. Additionally, Render's database URLs use a subtle difference in credential generation—I initially had authentication failures because the password contained characters that looked similar (digit '1' vs lowercase 'l'), causing copy-paste errors." 
        sectionTitle="Database Migration Details" 
      />
      <TextBlock 
        text="Data migration used an API-based approach rather than direct database connection. I wrote upload_to_render.py that reads the local JSON file with 1,249 activities and embeddings, chunks them into batches of 100, and posts to the /activities/bulk-upload endpoint. This approach avoided database connection complexity and verified that the production API endpoints worked correctly for bulk operations. The script included verification: after upload, it queries /activities to confirm all 1,249 activities were stored correctly." 
        sectionTitle="API-Based Data Migration" 
      />
      <TextBlock 
        ...
        sectionTitle="Deployment Configuration" 
      />
      <TextBlock 
      ...
      />
      <TextBlock 
        text="TypeScript configuration caused unexpected build failures on Render that never appeared locally. Render's build environment is stricter about type definitions and module resolution. The solution loosened TypeScript strictness for cloud builds: disabled skipLibCheck, added allowSyntheticDefaultImports, and removed problematic type references. The development environment still enforces strict TypeScript for local development; production builds use more lenient settings. This pragmatic compromise kept type safety during development while allowing successful cloud deployment." 
        sectionTitle="TypeScript Build Issues" 
      />
      <TextBlock 
        text="Dependency management required careful restructuring. Locally, build tools like Vite, Tailwind, and PostCSS can live in devDependencies. Render's production builds need these in regular dependencies because the build happens in the production environment. Moving vite, @vitejs/plugin-react, autoprefixer, postcss, and tailwindcss to dependencies resolved 'command not found' errors during Render's build process. Only development-only tools like ESLint and TypeScript type definitions stayed in devDependencies." 
        sectionTitle="Dependency Restructuring" 
      />
      <TextBlock 
        ...
        sectionTitle="Migration Outcomes" 
      />
    </Article>
  );
};
